...
func getGBPTransactionsFromBackRequest(page: Int, size: Int, completion: @escaping (_ transactionsResponse: NSDictionary) -> Void)
{
    makeGetRequest(endpointURL: "transactions", paramsURL: "page="+String(page)+"&size="+String(size), completion: {completionDictionary in
        ...
        if let transactions = completionDictionary["transactions"] {
            ...
            if (transactions as AnyObject).value(forKeyPath: "content") != nil {
                ...
                completion(["status": true, "newTransactions": content.count] as NSDictionary)
            }
        } else if let errorMessage = completionDictionary["errorMessage"] {
            completion(["status": false, "errorMessage": errorMessage] as NSDictionary)
        } else {
